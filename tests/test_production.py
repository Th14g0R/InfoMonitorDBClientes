...
class ProductionConfigTests(unittest.TestCase):
    ...
    def test_instaladores_usam_runner_lock_e_diretorio_absoluto(self):
        ...
        self.assertIn("'update'", windows[:windows.index('$ErrorActionPreference')])
        self.assertIn('[1] Instalar novo', windows)
        self.assertIn('[2] Atualizar existente', windows)
        self.assertIn('function Assert-RequestedMode', windows)
        self.assertIn("Prepare-And-Apply $Target 'install'", windows)
        self.assertIn("Prepare-And-Apply $Target 'update'", windows)
        self.assertIn("Join-Path $Prepared 'elevated-result.txt'", windows)
        self.assertIn('ERRO ELEVADO:', windows)
        self.assertIn('$nssm = Find-Nssm $Folder', windows)
        self.assertIn('A janela elevada nao registrou detalhes', windows)
        self.assertIn('SUCESSO: $operationLabel concluida em $Target', windows)
        ...
        self.assertIn("'install' { Prepare-And-Apply $Target 'install' }", windows)
        self.assertIn("'update' { Prepare-And-Apply $Target 'update' }", windows)
        ...
        self.assertIn('mode=$Mode', prepare_slice)
        ...
        self.assertIn("$metadata.mode -eq 'update'", apply_slice)
